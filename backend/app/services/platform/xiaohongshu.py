"""Xiaohongshu login via SAU-native qrcode_callback.

Spawns xiaohongshu_setup(handle=True, qrcode_callback=...) in a
background asyncio task. SAU writes the QR PNG to disk; we forward
the bytes to the frontend via the platform login API.
"""
...
class XiaohongshuLogin(PlatformLoginBase):
    ...
    async def start_qr_login(self) -> bytes:
        ...
        async def _runner():
            try:
                ...
                logger.error("xiaohongshu SAU setup crashed: %s", e)
        ...
        if not self._latest_qr_bytes:
            raise RuntimeError("小红书二维码生成超时（5s）")
        return self._latest_qr_bytes
    ...
    async def check_login_status(self) -> bool:
        ...
        try:
            ...
            logger.warning("xiaohongshu check_login_status: %s", e)
            return False
