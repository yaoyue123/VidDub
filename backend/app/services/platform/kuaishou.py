"""Kuaishou login via SAU-native qrcode_callback.

Spawns ks_setup(handle=True, qrcode_callback=...) in a background
...
class KuaishouLogin(PlatformLoginBase):
    ...
    async def start_qr_login(self) -> bytes:
        ...
        async def _runner():
            try:
                ...
                logger.error("kuaishou SAU setup crashed: %s", e)
        ...
        if not self._latest_qr_bytes:
            raise RuntimeError("快手二维码生成超时（5s）")
        return self._latest_qr_bytes
    ...
    async def check_login_status(self) -> bool:
        ...
        try:
            ...
            logger.warning("kuaishou check_login_status: %s", e)
            return False
