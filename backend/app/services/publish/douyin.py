...
class DouyinPublisher(PlatformPublisher):
    """Douyin publisher wrapping SAU's DouYinVideo Playwright uploader.

    Usage flow:
        1. Cookie: reads SAU account file (douyin_{name}.json)
        2. Upload: DouYinVideo → Playwright browser → douyin creator portal
    """
    ...
    async def validate_login(self) -> bool:
        ...
        if not os.path.exists(self.storage_state_path):
            logger.warning("Douyin account file missing: %s", self.storage_state_path)
            return False

        try:
            # SAU's douyin_setup does cookie validation
            from uploader.douyin_uploader.main import douyin_setup

            result = await douyin_setup(
                self.storage_state_path,
                handle=False,  # don't re-login, just check
                return_detail=True,
                headless=True,
            )
            return bool(result.get("success"))
        except ImportError as e:
            logger.warning("SAU douyin uploader import error: %s", e)
            return False
        except Exception as e:
            logger.warning("Douyin validate_login exception: %s", e)
            return False
    ...
    async def publish(
        ...
        """Publish video to Douyin via SAU's DouYinVideo."""
        ...
        if not os.path.exists(self.storage_state_path):
            return PublishResult(
                success=False,
                error_msg="未登录抖音 (account file 缺失)",
                needs_relogin=True,
            )
        ...
        try:
            ...
            # Build SAU-compatible tags (remove # prefix if any, limit length)
            ...
            # Run SAU upload (it manages its own Playwright browser)
            ...
            logger.info("Douyin publish success for video %d", video_id)

            return PublishResult(
                success=True,
                platform_video_url=f"https://creator.douyin.com/creator-micro/content/manage",  # 管理页
            )

        except ImportError as e:
            logger.error("SAU douyin import error: %s", e)
            return PublishResult(
                success=False,
                error_msg=f"social-auto-upload 抖音模块不可用: {e}",
            )
        except Exception as e:
            logger.error("Douyin publish error: %s", e, exc_info=True)
            error_str = str(e)
            needs_relogin = any(
