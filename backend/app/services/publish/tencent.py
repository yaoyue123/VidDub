"""Tencent (WeChat Channels) publisher using social-auto-upload's TencentVideo class.

Wraps SAU's Playwright-based tencent uploader into viddub's
PlatformPublisher interface. Uses SAU's cookie/account file format
at social-auto-upload/cookies/tencent_{account}.json.
"""
...
class TencentPublisher(PlatformPublisher):
    """WeChat Channels publisher wrapping SAU's TencentVideo Playwright uploader."""
    ...
    async def validate_login(self) -> bool:
        """Check if tencent account file is valid using SAU's cookie_auth."""
        if not os.path.exists(self.storage_state_path):
            logger.warning("Tencent account file missing: %s", self.storage_state_path)
            return False
        ...
        try:
            ...
            logger.warning("SAU tencent uploader import error: %s", e)
            return False
        except Exception as e:
            logger.warning("Tencent validate_login exception: %s", e)
            return False
    ...
    async def publish(
        ...
        """Publish video to WeChat Channels via SAU's TencentVideo."""
        ...
        if not os.path.exists(self.storage_state_path):
            return PublishResult(
                success=False,
                error_msg="未登录微信视频号 (account file 缺失)",
                needs_relogin=True,
            )
        ...
        try:
            ...
            logger.info("Tencent publish success for video %d", video_id)

            return PublishResult(
                success=True,
                platform_video_url="https://channels.weixin.qq.com/",
            )

        except ImportError as e:
            logger.error("SAU tencent import error: %s", e)
            return PublishResult(
                success=False,
                error_msg=f"social-auto-upload 微信视频号模块不可用: {e}",
            )
        except Exception as e:
            logger.error("Tencent publish error: %s", e, exc_info=True)
            error_str = str(e)
            needs_relogin = any(
            ...
            return PublishResult(
                success=False,
                error_msg=f"微信视频号发布失败: {error_str}",
