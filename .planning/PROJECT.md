...
## Completed Milestone: v5.0 开源化深度改造 ✅ (2026-06-29)

**Goal:** 把项目从"能跑但混乱的内部工具"改造为"结构清晰、文档完善、便于社区使用和贡献的开源项目"。功能不变，但代码质量、可读性、文档完整度大幅提升，达到可开源标准。

**Outcome:** 6/6 phases complete. 仓库结构干净、API 规范化、配置统一、文档齐全。

---

## Completed Milestone: v5.1 Discover 页面重构与 Bug 修复 ✅ (2026-07-01)

**Goal:** 将 discover 页面从旧推荐系统重构为多维度内容跟踪系统（关键词/博主/playlist/热点/主题），支持手动搜索与自动扫描，并修复 YouTube 封面不显示和下载目录重复问题。

**Outcome:** 5/5 phases complete. 全栈重构：基础设施（WAL 模式、YtDlpWrapper、限流器）→ 发现扫描协调器 → 跟踪 API → Discover 多标签 UI → Bug 修复。

---

## Current Milestone: v5.2 音色仿写支持

**Goal:** 在语音转写过程中自动提取原视频说话人的音频片段作为音色参考样本，通过 SiliconFlow API 的 `references` 参数实现说话人音色克隆，让配音保留原说话人的声音特征。

**Target features:**
- **自动音色提取** — 转写时从原视频中自动截取一段干净的人声片段作为参考样本
- **音色克隆 TTS** — CosyVoice2 TTS 管线支持传参 `references` 实现说话人克隆
- **配置项** — 任务创建/编辑页面增加音色选择下拉框（预设音色 / 自动克隆原声）
...
- ✓ Phase 11-12: UX 重设计
- ✓ v5.0: 开源化深度改造 — 目录结构清理、API 规范化、配置统一、死代码清除、开源文档、构建脚本整合

### Active (v5.2 音色仿写支持)

<!-- Current scope. Building toward these. -->

- [ ] 自动音色提取 — 转写时从原视频自动截取干净人声片段
- [ ] 音色克隆 TTS — CosyVoice2 支持 `references` 参数实现说话人克隆
- [ ] 音色选择 UI — 任务配置中增加音色选择下拉框
...
- 多用户系统 — 个人自用工具
- 移动端 App — 仅 Web 界面
- v4.0 五维评分引擎 — v5.1 采用简单筛选替代，不做智能评分
- v4.0 自定义规则引擎 — 不在本 milestone 范围内
...
---
*Last updated: 2026-07-01 — v5.2 音色仿写支持 milestone started (v5.1 completed)*
